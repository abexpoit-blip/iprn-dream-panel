...
let isActive = false;
let BOT_ID = null;
const BOT_NAME = 'SMS Hadi Bot';
const BOT_TYPE = 'smshadi';
...
async function resolvePanelMode() {
  ...
  console.log(`[smshadi-bot] PANEL_MODE=${PANEL_MODE}`);
}
...
async function verifySession(origin) {
  ...
  console.log(`[smshadi-bot] verifySession probe=${probe} status=${r.status} redirect=${loc || '-'} bodyLen=${bodyStr.length} ok=${ok}`);
  if (!ok && bodyStr.length > 0 && bodyStr.length < 400) {
    console.log(`[smshadi-bot] verifySession body preview: ${bodyStr.slice(0, 200).replace(/\s+/g, ' ')}`);
  }
  return ok;
}
...
async function login() {
  ...
  const url  = await getSetting(BOT_ID, 'portal_url', 'http://2.59.169.96/ints/login');
  ...
  if (sessionCookie && sessionCookie.trim().length > 5) {
    ...
    console.log(`[smshadi-bot] Using pasted session cookie (${n} entries) — skipping login form`);
    if (await verifySession(origin)) {
      console.log(`[smshadi-bot] Session cookie verified — logged in as ${PANEL_MODE}`);
      await updateBotStatus('online', null);
      return true;
    }
    const reason = 'Pasted session cookie rejected. Paste a fresh PHPSESSID.';
    console.error(`[smshadi-bot] ${reason}`);
    await updateBotStatus('offline', reason);
    return false;
  }

  console.log(`[smshadi-bot] Attempting login for ${user} at ${url} (mode=${PANEL_MODE})`);
  try {
    ...
    if (manualCaptcha && manualCaptcha.trim()) {
      ...
      console.log(`[smshadi-bot] Solved math captcha: ${form.mathMatch[1]}+${form.mathMatch[2]}=${captchaResult}`);
    ...
    if (ok) {
      console.log(`[smshadi-bot] Login successful (status=${res.status}, path=${finalPath})`);
      await updateBotStatus('online', null);
      return true;
    }
    ...
    console.error(`[smshadi-bot] Login error:`, err.message);
...
async function scrapeNumbers() {
  ...
  const loginUrl = await getSetting(BOT_ID, 'portal_url', 'http://2.59.169.96/ints/login');
  ...
  try {
    ...
    if (res.status !== 200) {
      console.error(`[smshadi-bot] Numbers HTTP ${res.status}`);
    ...
    if (items.length === 0) {
      console.log(`[smshadi-bot] Numbers scrape: 0 rows from ${url} (panel has no numbers yet)`);
      return;
    }
    ...
    try {
      ...
      console.error('[smshadi-bot] stale-number cleanup error:', e.message);
    }
    console.log(`[smshadi-bot] Numbers scrape: ${items.length} parsed, ${inserted} upserted, ${removedCount} removed (no longer in panel)`);
  } catch (err) {
    console.error(`[smshadi-bot] Numbers scrape error:`, err.message);
  }
}
...
async function alreadyLogged(sourceMsgId) {
  try {
    const row = await db.prepare(
      `SELECT 1 FROM otp_audit_log WHERE source = 'smshadi' AND source_msg_id = ? LIMIT 1`
    ).get(sourceMsgId);
    return !!row;
...
async function scrapeSms() {
  ...
  try {
    ...
    if (res.status !== 200) {
      console.error(`[smshadi-bot] CDR HTTP ${res.status}`);
      if (res.status === 401 || res.status === 302) await login();
      return;
    }
    const data = typeof res.body === 'object' ? res.body : JSON.parse(res.body);
    ...
    console.log(`[smshadi-bot] CDR scrape: rows=${realRows.length}, billed=${billed}, dup=${dup}`);
  } catch (err) {
    console.error(`[smshadi-bot] CDR scrape error:`, err.message);
  }
}
...
async function start() {
  isActive = true;
  console.log('[smshadi-bot] Bot starting (CLIENT mode)...');
  ...
  try {
    ...
    console.log(`[smshadi-bot] BOT_ID=${BOT_ID}`);
  } catch (err) {
    console.error('[smshadi-bot] Failed to resolve BOT_ID:', err.message);
    return;
  }
  ...
  try {
    ...
    await sql.listen('scrape_now', () => {
      console.log('[smshadi-bot] [auto-pool] NOTIFY scrape_now received');
      scrapeNumbers();
    });
    console.log('[smshadi-bot] [auto-pool] listening on channel scrape_now');
  } catch (e) {
    console.error('[smshadi-bot] [auto-pool] LISTEN failed:', e.message);
  }
}
